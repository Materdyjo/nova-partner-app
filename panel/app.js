...
const state = {
  ...
  reservationSearch: '',     // filtr tekstowy (klient / kod / dokument) w rezerwacjach
};
...
/**
 * Status rezerwacji (decyzja Nova Seafood 2026-09-03): RO w buforze Optimy =
 * „Do realizacji" (to, co dopiero będzie wysłane); RO poza buforem =
 * „Zrealizowana"; anulowana w Optimie = „Anulowana". Bez statusów personelu.
 */
...
async function enterApp(user) {
  ...
  // Bez parametru: wracamy do ostatnio otwartej zakładki (zapamiętanej
  // w przeglądarce), żeby odświeżenie strony nie cofało do Promocji.
  let remembered = null;
  try { remembered = localStorage.getItem('panel.view'); } catch { /* brak storage */ }
  const wanted = new URLSearchParams(location.search).get('view') || remembered;
...
function switchView(view) {
  state.view = view;
  try { localStorage.setItem('panel.view', view); } catch { /* brak storage */ }
  $$('.nav-item').forEach((b) =>
...
/**
 * Czyta CAŁĄ listę stronami po 1000 wierszy — pojedyncze zapytanie zwraca
 * maks. 1000 (limit PostgREST), a klientów jest ponad 2000; bez tego
 * klienci z końca alfabetu (np. WIKOR) nie pojawiali się w panelu.
 * `build` zwraca świeże zapytanie (select + order) — range dokłada helper.
 */
...
async function openPromoModal(promo) {
  ...
  const { root, close } = openModal(isEdit ? 'Edytuj promocję' : 'Nowa promocja', `
    <form id="promo-form" class="form">
      ...
      <label class="field">Cena promocyjna netto [zł]*
        <input type="number" id="pf-price" step="0.01" min="0.01" required value="${esc(promo?.promo_net_price ?? '')}">
        ${promo?.discount_pct != null ? `<span class="muted small">Ta promocja była zapisana jako rabat ${esc(fmtPct(promo.discount_pct))}% — promocje procentowe zostały wycofane (22.09.2026), wpisz cenę.</span>` : ''}
      </label>
      <div class="field-row">
  ...
  $('#promo-form', root).addEventListener('submit', async (e) => {
    e.preventDefault();
    // Decyzja właściciela (22.09.2026): promocje WYŁĄCZNIE jako cena netto —
    // rabaty procentowe wycofane (discount_pct zawsze null; CHECK XOR w tabeli
    // promotions nadal spełniony).
    const price = parseFloat($('#pf-price', root).value);
    if (!(price > 0)) return toast('Podaj cenę promocyjną większą od zera.');
...
// ── Widok: Aktywność (logowania klientów + przeglądane produkty) ────────────
// Logowania czyta z auth_events (pisze je pin-login/pin-admin; staff ma SELECT
// przez RLS). Przeglądane produkty z product_views — tabela może jeszcze nie
// istnieć (zdarzenia zacznie wysyłać dopiero zaktualizowana aplikacja), więc
// błąd zapytania pokazujemy jako informację, nie jako awarię widoku.
const ACTIVITY_EVENT_LABELS = {
  login_ok: ['Zalogował się', 'badge--ok'],
  login_fail: ['Nieudane logowanie', 'badge--danger'],
  locked: ['Blokada po nieudanych próbach', 'badge--danger'],
  pin_set: ['PIN nadany', 'badge--info'],
  pin_revoked: ['PIN cofnięty', 'badge--muted'],
  device_bound: ['Nowe urządzenie powiązane', 'badge--info'],
  device_blocked: ['Odrzucone obce urządzenie', 'badge--danger'],
  devices_unlocked: ['Urządzenia odblokowane', 'badge--muted'],
};
...
async function renderActivity() {
  ...
  const [eventsRes, phonesRes, viewsRes] = await Promise.all([
    supabase.from('auth_events')
      .select('phone, event, created_at')
      .order('created_at', { ascending: false })
      .limit(200),
    fetchAllRows(() => supabase.from('customers').select('code, name, phone').not('phone', 'is', null).order('id')),
    supabase.from('product_views')
      .select('viewed_at, product_name, products(code, name), customers(code, name)')
  ...
  if (eventsRes.error) {
    view.innerHTML = errorHtml();
    return toastError('Błąd pobierania aktywności', eventsRes.error);
  }

  const byPhone = new Map();
  for (const c of phonesRes.data ?? []) byPhone.set(c.phone, c);
  const who = (phone) => {
    const c = byPhone.get(phone);
    return c
      ? `${esc(c.name)} <span class="muted">(${esc(c.code ?? '—')})</span>`
      : '<span class="muted">nieznany numer</span>';
  };

  const evRow = (e) => {
    const [label, cls] = ACTIVITY_EVENT_LABELS[e.event] ?? [e.event, 'badge--muted'];
    return `<tr>
      <td class="nowrap">${fmtDateTime(e.created_at)}</td>
      <td>${who(e.phone)}</td>
      <td class="nowrap">${esc(e.phone ?? '—')}</td>
      <td><span class="badge ${cls}">${esc(label)}</span></td>
    </tr>`;
  };

  const viewsBlock = viewsRes.error
    ? '<p class="muted small">Rejestr przeglądanych produktów ruszy po najbliższej aktualizacji aplikacji klienta.</p>'
    : ((viewsRes.data ?? []).length === 0
      ? emptyHtml('Brak zarejestrowanych wyświetleń produktów.')
      : `<div class="table-wrap"><table>
          <thead><tr><th>Kiedy</th><th>Klient</th><th>Produkt</th></tr></thead>
          <tbody>${viewsRes.data.map((v) => `
            <tr>
              <td class="nowrap">${fmtDateTime(v.viewed_at)}</td>
              <td>${v.customers ? `${esc(v.customers.name)} <span class="muted">(${esc(v.customers.code ?? '—')})</span>` : '—'}</td>
              <td>${v.products ? `${esc(v.products.name)} <span class="muted">(${esc(v.products.code)})</span>` : esc(v.product_name ?? '—')}</td>
            </tr>`).join('')}</tbody>
        </table></div>`);

  view.innerHTML = `
    <header class="view-head"><h1>Aktywność</h1></header>
    <p class="muted small">Zdarzenia kont klientów (ostatnie 200) oraz przeglądane produkty.</p>
    <h2>Logowania</h2>
    ${(eventsRes.data ?? []).length === 0 ? emptyHtml('Brak zdarzeń logowania.') : `
      <div class="table-wrap"><table>
        <thead><tr><th>Kiedy</th><th>Klient</th><th>Numer</th><th>Zdarzenie</th></tr></thead>
        <tbody>${eventsRes.data.map(evRow).join('')}</tbody>
      </table></div>`}
    <h2>Przeglądane produkty</h2>
    ${viewsBlock}`;
}
...
async function renderReservations() {
  ...
  view.innerHTML = `
    <header class="view-head">
      <h1>Rezerwacje</h1>
      <label class="filter">Szukaj
        <input type="search" id="res-search" placeholder="klient / kod / dokument"
               value="${esc(state.reservationSearch ?? '')}">
      </label>
    ...
    <p class="muted small">Rezerwacje pochodzą z Optimy (tylko odczyt). Status: w buforze Optimy → <strong>Do realizacji</strong>, poza buforem → <strong>Zrealizowana</strong>, anulowana → Anulowana. Klient widzi w aplikacji tylko „Do realizacji". Personel ustawia planowaną dostawę.</p>
    ${data.length === 0 ? emptyHtml('Brak rezerwacji.') : `
      <div class="table-wrap">
        <table>
          <thead><tr>
            <th>Dokument</th><th>Klient</th><th>Status</th><th>Optima</th>
            <th>Data dok.</th><th>Rezerwacja do</th>
            <th>Netto</th><th>Planowana dostawa</th>
          </tr></thead>
          <tbody>${data.map((r) => {
            const eff = effectiveResStatus(r);
            const hay = `${r.customers?.name ?? ''} ${r.customers?.code ?? ''} ${r.document_number ?? ''}`.toLowerCase();
            return `
            <tr data-search="${esc(hay)}">
  ...
  // Wyszukiwanie tekstowe: filtruje załadowane wiersze bez przeładowania
  // widoku (pole nie traci fokusu w trakcie pisania).
  const applyResSearch = (raw) => {
  ...
  $('#res-search')?.addEventListener('input', (e) => {
    state.reservationSearch = e.target.value;
    applyResSearch(e.target.value);
  });
  applyResSearch(state.reservationSearch);
...
async function renderCustomers() {
  ...
  const [custRes, pinsRes, catRes, devRes, repsRes] = await Promise.all([
    fetchAllRows(() => supabase
      .from('customers')
      .select('id, code, name, nip, phone, segment, price_level, is_active, max_devices, sales_rep_name, sales_rep_phone, sales_rep_email, customer_users(count)')
      .order('name')
      .order('id')),
    ...
    // Słownik opiekunów (migracja 0014): telefon/e-mail per nazwa z Optimy.
    supabase.from('sales_reps').select('name, phone, email').order('name'),
  ]);
  if (token !== viewToken) return;
  if (custRes.error) {
    view.innerHTML = errorHtml();
  ...
  view.innerHTML = `
    ...
    <p class="muted small">Dane klientów płyną z Optimy (tylko odczyt). W panelu zarządzasz PIN-ami logowania i autoryzacją urządzeń. Cennik widzi każdy klient (produkty z dostępnością 0 kg są ukryte automatycznie).</p>
    <div id="reps-card"></div>
    <div id="cust-table"></div>`;

  // ── Opiekunowie handlowi ──────────────────────────────────────────────────
  // Nazwa opiekuna przychodzi z Optimy (atrybut OPIEKUN). Telefon/e-mail
  // wpisujemy RAZ per opiekun — triggery z migracji 0014 kopiują je na
  // wszystkich jego klientów (także po każdym syncu).
  const repsMap = new Map();
  if (!repsRes.error) for (const r of repsRes.data ?? []) repsMap.set(r.name.trim().toLowerCase(), r);
  const repNames = new Map(); // klucz -> { name, count }
  for (const c of data) {
    const n = (c.sales_rep_name ?? '').trim();
    if (!n) continue;
    const k = n.toLowerCase();
    const e = repNames.get(k);
    if (e) e.count++; else repNames.set(k, { name: n, count: 1 });
  }
  const noRep = data.filter((c) => !(c.sales_rep_name ?? '').trim()).length;
  const repsEl = $('#reps-card');
  if (repsRes.error) {
    repsEl.innerHTML = `<div class="card"><h2>Opiekunowie handlowi</h2><p class="muted small">Brak tabeli opiekunów — uruchom migrację 0014 w Supabase.</p></div>`;
  } else {
    const rows = [...repNames.values()].sort((a, b) => a.name.localeCompare(b.name, 'pl'));
    repsEl.innerHTML = `
      <div class="card">
        <h2>Opiekunowie handlowi</h2>
        <p class="muted small">Nazwisko opiekuna pochodzi z Optimy (atrybut kontrahenta OPIEKUN). Tu wpisujesz raz telefon i e-mail opiekuna — trafią do wszystkich jego klientów, także nowych po syncu. Telefon w formacie +48…${noRep ? ` · <strong>${noRep}</strong> klientów bez opiekuna w Optimie.` : ''}</p>
        ${rows.length === 0 ? '<p class="muted">Żaden klient nie ma jeszcze przypisanego opiekuna w Optimie.</p>' : `
        <div class="table-wrap">
          <table>
            <thead><tr><th>Opiekun (Optima)</th><th>Klientów</th><th>Telefon</th><th>E-mail</th><th></th></tr></thead>
            <tbody>${rows.map((r) => {
              const cur = repsMap.get(r.name.toLowerCase()) ?? {};
              return `
              <tr data-rep="${esc(r.name)}">
                <td><strong>${esc(r.name)}</strong></td>
                <td>${r.count}</td>
                <td><input type="tel" data-rep-phone value="${esc(cur.phone ?? '')}" placeholder="+48…" style="min-width:150px"></td>
                <td><input type="email" data-rep-email value="${esc(cur.email ?? '')}" placeholder="imie@novaseafood.pl" style="min-width:200px"></td>
                <td class="row-actions"><button type="button" class="btn btn-ghost" data-rep-save>Zapisz</button></td>
              </tr>`;
            }).join('')}</tbody>
          </table>
        </div>`}
      </div>`;
    $$('[data-rep-save]', repsEl).forEach((b) => b.addEventListener('click', async () => {
      const tr = b.closest('tr');
      const name = tr.dataset.rep;
      const phone = $('[data-rep-phone]', tr).value.trim();
      const email = $('[data-rep-email]', tr).value.trim();
      if (phone && !/^\+\d{8,15}$/.test(phone)) return toast('Telefon w formacie międzynarodowym, np. +48601234567.', 'error');
      b.disabled = true;
      try {
        const { error } = await supabase.from('sales_reps').upsert({ name, phone: phone || null, email: email || null }, { onConflict: 'name' });
        if (error) throw error;
        toast(`Zapisano opiekuna ${name}.`, 'ok');
      } catch (err) {
        toastError('Nie udało się zapisać opiekuna', err);
      } finally {
        b.disabled = false;
      }
    }));
  ...
  // Limit per klient (customers.max_devices, migracja 0013); null = globalny 1.
  const deviceLimit = (c) => (c.max_devices ?? 1);
  const deviceCell = (c) => {
    if (!devices) return '<span class="muted">—</span>';
    const list = devices.get(c.id) ?? [];
    const limitLine = `<div class="muted small">${list.length} / ${deviceLimit(c)}${c.max_devices ? ' <span class="badge badge--info">limit własny</span>' : ''}</div>`;
    if (list.length === 0) return `<span class="muted">brak</span>${limitLine}`;
    return list.map((d) => `
  ...
  const rowActions = (c) => {
    ...
    parts.push(`<button type="button" class="btn btn-ghost" data-devices-limit="${esc(c.id)}">Limit urządzeń</button>`);
  ...
  // Limit urządzeń per klient (np. kontrahent pokazowy na targi = 4).
  // Zapis bezpośrednio do customers.max_devices (grant kolumnowy dla staffa,
  // polityka customers_staff_update). Pusta wartość = powrót do globalnego 1.
  async function onDevicesLimit(btn) {
    const c = data.find((x) => x.id === btn.dataset.devicesLimit);
    if (!c) return;
    const raw = prompt(`Limit urządzeń dla „${c.name}" (1–20).\nZostaw puste, aby wrócić do domyślnego (1).`, c.max_devices ?? '');
    if (raw === null) return;
    ...
    if (trimmed !== '') {
      ...
      if (!Number.isInteger(value) || value < 1 || value > 20) {
        return toast('Podaj liczbę od 1 do 20.', 'error');
      }
    }
    btn.disabled = true;
    try {
      const { error } = await supabase.from('customers').update({ max_devices: value }).eq('id', c.id);
      if (error) throw error;
      toast(value === null ? 'Przywrócono domyślny limit (1).' : `Limit urządzeń: ${value}.`, 'ok');
      renderCustomers();
    } catch (err) {
      btn.disabled = false;
      toastError('Nie udało się zapisać limitu', err);
    }
  }
  ...
  function renderTable(filterText) {
      ...
      : `<div class="table-wrap">
          <table>
            <thead><tr>
              ...
              <th>Poziom cen</th><th>Aktywny</th>
    ...
    $$('[data-devices-limit]', tableEl).forEach((b) =>
      b.addEventListener('click', () => onDevicesLimit(b)));
  }
