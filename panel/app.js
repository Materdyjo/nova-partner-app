...
async function renderActivity() {
  ...
  const [eventsRes, phonesRes, viewsRes] = await Promise.all([
    supabase.from('auth_events')
      ...
      .order('created_at', { ascending: false })
      .limit(200),
    supabase.from('customers').select('code, name, phone').not('phone', 'is', null).limit(2000),
...
async function renderReservations() {
  ...
  view.innerHTML = `
    ...
    <p class="muted small">Baza rezerwacji pochodzi z Optimy (tylko odczyt: bufor / anulowana). Status dla klienta i planowaną dostawę ustawia personel — „auto" oznacza status wyliczony (bufor → Do realizacji, inaczej Potwierdzona).</p>
